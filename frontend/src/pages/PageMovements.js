...
import {
  ArrowDownUp, ArrowUp, ArrowDown, ArrowUpDown,
...
import { fetchMouvements, fetchFiltres, fetchBases, fetchStock } from '../api/stockApi';
      ...
      <div className="relative w-full">
        <button
          ...
          className={`
            ...
            ${open ? 'border-[#12a6e0] shadow-[0_0_0_3px_rgba(18,166,224,0.12)]'
                   : isDisabled ? 'border-[#e0e0e0] bg-[#f5f5f5]'
...
function exportCSV(data, cols) {
  ...
  a.href=url; a.download=`mouvements_${new Date().toISOString().slice(0,10)}.csv`; a.click();
...
export default function PageMovements() {
  ...
  const gridCols = isMobile ? 'grid-cols-1' : isTablet ? 'grid-cols-2' : 'grid-cols-[repeat(auto-fill,minmax(240px,1fr))]';
          ...
          <div className="bg-white border border-[#e8e8e8] rounded-2xl overflow-hidden shadow-[0_1px_6px_rgba(0,0,0,0.06)]">
            ...
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#f0f0f0]">
              ...
              {!loading && mouvData?.length > 0 && (
                <button onClick={() => exportCSV(sorted, cols)}
                ...
                <div className="flex items-center gap-1">
                  ...
                  {Array.from({length:Math.min(5,totalPages)},(_,i)=>{
                    ...
                    return <button key={p} onClick={()=>setPage(p)}
                      className={`w-8 h-8 rounded-lg text-[0.75rem] cursor-pointer transition-all ${page===p?'bg-[#12a6e0] text-white font-semibold border-0':'bg-[#f5f5f5] text-[#666666] border border-[#e0e0e0] hover:bg-[#eaeaea]'}`}>{p}</button>;
                  })}
